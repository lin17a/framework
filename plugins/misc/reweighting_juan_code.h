...
Number2 calc_interference_scalar(event_t<Number2> event){
     ...
     switch (event.version){
          ...
          case calc_weight_version::juan_paper:{
              ...
              interference_scalar = factor_interference * event.beta_ref
...
Number2 calc_no_interference_term(event_t<Number2> event){
    ...
    switch (event.version){ 
        case calc_weight_version::juan_paper:
            additional_factor_for_nointerf = 8 * pow(constants<Number2>::m_t_ref, 2) * TMath::Pi() * pow(constants<Number2>::alpha_s, 2) * event.beta_ref / pow(event.s, 2);
            break;
        case calc_weight_version::juan_code: 
...
Number2 calc_qcd_opp_gluon(event_t<Number2> event){
    ...
    switch (event.version){
        case calc_weight_version::juan_paper:
            qcd_term = 2 * pow(constants<Number2>::alpha_s, 2) * pow(event.beta_ref, 3) * TMath::Pi() / event.s * common_factor_for_M2_gg_QCD * (1 - event.z_sq) 
