...

{
  ...
  while ((key1=(TKey*)iter1.Next()) ) {
    ...
    try {
        ...
        TH1F *h_sum_ttbarlo_res_int_pseudo = (TH1F*) h1->Clone();
        h_sum_ttbarlo_res_int_pseudo->Add(h2);
        //h_sum_ttbarlo_res_int_pseudo->Add(h4);
        h_sum_ttbarlo_res_int_pseudo->SetLineColor(kAzure+3);
	...
	if (unit != units.end()) {
        ...
        //h4->Draw("pesame");
        //h5->Draw("pesame");
        h_sum_ttbarlo_res_int_pseudo->Draw("pe");  
        h1->Draw("pesame");

        
        // add a sub pad with the ratio 
        //auto rp1 = new TRatioPlot(h1, h_sum_ttbarlo_res_int_pseudo);
        //rp1->Draw();
